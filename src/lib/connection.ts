...
export class PeerJSGame {
	...
	private callbacks: Map<
		GameEvent,
		{ callback: (clientId: string, data: any) => void; lifetime: number }[]
	> = new Map();
	...
	async connect(pub: boolean, id?: string): Promise<void> {
		...
		const response: JoinLobbyResponse = await (
			await fetch(url, {
				method: 'POST',
				headers: {
	...
	/**
	 * Helper: Registers a one-time event listener for the specified event.
	 *
	...
	private parse(data: string): GameMessage {
		const message: GameMessage = GameMessageSchema.json().cast(data);
	...
	private handleCallbacks(message: GameMessage, connection: DataConnection): void {
		...
		this.callbacks.set(
			message.event,
			this.callbacks.get(message.event)!.filter((val) => val.lifetime !== 0)
		);
	}
}
